...
var totalFrames = 45; // the number of images in the sequence of JPEG files (this could be calculated server-side by scanning the frames folder)
...
function changeFrame() {
	...
	if(images.length > 0 && images[thisStep]) { // if the image exists in the array
		if(images[thisStep].complete) { // if the image is downloaded and ready
			$('#video, #video2').attr('src',images[thisStep].src); // change the source of our placeholder image
		}
	}
}

function resizeAdjustments() { // fit everything to the screen
	$('.m1').css('height',(totalFrames*30)+'px'); // increase the height of the document 30 pixels for every frame in the JPEG sequence
